...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    wp_enqueue_script('jquery', get_template_directory_uri().'/libs/jquery.min.js','','',true); // добавляем свой
	    wp_enqueue_script('bootstrap', get_template_directory_uri().'/libs/bootstrap.bundle.min.js','','',true); // бутстрап
	    wp_enqueue_script('dselect', get_template_directory_uri().'/libs/dselect.js','','',true); // dselect
	    // wp_enqueue_script('sticky', get_template_directory_uri().'/libs/sticky-sidebar.js','','',true); // sticky
	    // wp_enqueue_script('resize', get_template_directory_uri().'/libs/resizeSencor.js','','',true); // resize
	    // wp_enqueue_script('intel', get_template_directory_uri().'/libs/intlTelInput.min.js','','',true); // tel
	    wp_enqueue_script('keen', get_template_directory_uri().'/libs/keen-slider.min.js','','',true); // и скрипты шаблона
	    wp_enqueue_script('main', get_template_directory_uri().'/js/swiper-course.js','','',true); // Основной
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
	    // wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/libs/bootstrap_5_0_2.min.css' ); // бутстрап
	    wp_enqueue_style( 'keen', get_template_directory_uri().'/libs/keen-slider.min.css' ); // keen
	    wp_enqueue_style( 'dselect', get_template_directory_uri().'/libs/dselect.css' ); // dselect
	    // wp_enqueue_style( 'intel', get_template_directory_uri().'/libs/intlTelInput.min.css' ); // tel
		// wp_enqueue_style( 'main', get_template_directory_uri().'/css/app.css' ); // основные стили шаблона
		// wp_enqueue_style( 'io', get_template_directory_uri().'/style.css' ); // новые стили шаблона
	}
}
...
function disable_comments() {
    ...
    remove_post_type_support('category', 'comments');
    remove_post_type_support('post_tag', 'comments');
    remove_post_type_support('author', 'comments');
