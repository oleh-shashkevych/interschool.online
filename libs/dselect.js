...
function dselect(el, option = {}) {
    ...
    if (window.innerHeight > 474) {
        var defaultMaxHeight = "450px";
    } else {
        var defaultMaxHeight = (window.innerHeight - 94).toString() + "px";
    }
    //  const defaultMaxHeight = "450px";
    var direction = ""
    if (el.hasAttribute("data-dselect-direction")) {
        direction = el.getAttribute("data-dselect-direction");
    } else {
        direction = "dropdown";
    }
    const defaultSize = "";
    ...
    const maxHeight = defaultMaxHeight;
    ...
    function selectedTag(options, multiple) {
        if (multiple) {
          ...
          `);
            ...
            return isPlaceholder(selectedOption) ? `<span class="${classPlaceholder}">${selectedOption.getAttribute('data-inner')}</span>` : selectedOption.getAttribute('data-inner');
        }
    }
    ...
    function itemTags(options) {
        ...
        for (const option2 of options) {
            if (option2.tagName === "OPTGROUP") {
                ...
                const text = option2.getAttribute('data-inner');
                const title = option2.title;
                items.push(`<button${hidden} title="${title}" class="dropdown-item${active}" data-dselect-value="${value}" type="button" onclick="dselectUpdate(this, '${classElement}', '${classToggler}')"${disabled}>${text}</button>`);
            }
        }
        items = items.join("");
        return items;
    ...
    ` : "";
        const template = `
    <div class="${direction} ${classElement} ${additionalClass}">
      <button class="${classToggler} ${!el.multiple && clearable ? classTogglerClearable : ""}" title="${el.options[el.selectedIndex].title}" data-dselect-text="${!el.multiple && selectedText(el.options)}" type="button" data-bs-toggle="dropdown" aria-expanded="false"${autoclose}>
        ${selectedTag(el.options, el.multiple)}
      </button>
      <div class="dropdown-menu">
        <div class="d-flex flex-column">
          ${searchInput}
          <div class="dselect-items" style="max-height:${maxHeight};overflow-y:auto">
